...
# ─────────────────────────── SIDEBAR ────────────────────────────────
SUPPORTED_EXTS = (".csv", ".xlsx", ".xls", ".parquet", ".json", ".tsv", ".txt")


def _list_server_files():
    """Scan folders where user-attached files live, return list of paths."""
    import glob
    folders = ["/home/user/uploads", "/home/user/data_analyst_agent/data", "/home/user"]
    found = []
    seen = set()
    for folder in folders:
        for path in glob.glob(f"{folder}/*"):
            ...
            if not os.path.isfile(path):
                continue
            if path.lower().endswith(SUPPORTED_EXTS) and path not in seen:
                found.append(path)
                seen.add(path)
    return sorted(found)
...
with st.sidebar:
    ...
    # Option 1 — chat-attached files (bypasses upload proxy)
    server_files = _list_server_files()
    if server_files:
        st.markdown("**📂 Files attached to our chat:**")
        file_labels = ["— select a file —"] + [
            f"{p.split('/')[-1]}  ({p})" for p in server_files
        ]
        chosen_label = st.selectbox("Pick a file already on the server", file_labels)
        server_pick = None
        if chosen_label != "— select a file —":
            idx = file_labels.index(chosen_label) - 1
            server_pick = server_files[idx]
    else:
        server_pick = None
        st.caption("(No files attached yet — attach one to our chat, or upload below.)")

    st.divider()

    # Option 2 — traditional upload
    st.markdown("**⬆️ Or upload from your device:**")
    uploaded = st.file_uploader(
        "CSV / Excel / Parquet / JSON (up to 200 MB)",
        type=["csv", "xlsx", "xls", "parquet", "json", "tsv", "txt"],
    )
...
# Load data — priority: (1) server-picked file, (2) upload, (3) sample
if server_pick:
    with st.spinner(f"Loading {server_pick.split('/')[-1]}..."):
        try:
            df_raw, load_note = load_file(server_pick)
